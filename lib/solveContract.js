    ...
    for (var contract of contracts) {
        ...
        switch (contract.type) {
            ...
            case "Algorithmic Stock Trader I":ns.tprint(contract.type); algoStockTrade1(contract)
                break;
            case "Algorithmic Stock Trader II": ns.tprint(contract.type); algoStockTrade2(contract)
                break;
...
function algoStockTrade2(contract) {
    var profit = 0
    for (var i = 0; i < contract.data.length - 1; i++) {
        if(contract.data[i] < contract.data[i+1]){
            profit += contract.data[i+1] - contract.data[i]
            ns.tprint(contract.data[i]," -> ", contract.data[i+1], " = ",contract.data[i+1] - contract.data[i])
        }
    }
    //ns.tprint(contract.data)
    //ns.tprint(profit)
